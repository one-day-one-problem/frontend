  ...
  const fetchProblems = async () => {
    try {
      ...
      const response = await problemApi.getProblems({
        ...
        type: selectedType,
        sortType: sortBy,
        // 인증된 사용자만 미해결 문제 필터 적용
        onlyUnsolved: isAuthenticated ? onlyUnsolved : false,
      });
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <section className="bg-white rounded-xl p-4 sm:p-6 mb-6 sm:mb-8 shadow-sm">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
            ...
            {/* 표시 방식 선택 버튼 - 인증 상태와 무관하게 항상 동일한 위치에 표시 */}
          ...
          {/* 미해결 문제 필터 또는 로그인 유도 메시지 - 동일한 위치에 조건부 렌더링 */}
        ...
        {/* 문제 목록 섹션 - 고정 높이 설정으로 페이지 전환시 레이아웃 이동 방지 */}
        <div className="mb-8">
          {viewMode === "card" ? (
            <section className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 min-h-[500px]">
              {problemData?.problems.map((problem) => (
                <Link to={`/problems/${problem.id}`} key={problem.id}>
                  <motion.div
                    whileHover={{ scale: 1.02 }}
                    whileTap={{ scale: 0.98 }}
                    className="bg-white p-4 sm:p-5 rounded-xl shadow-sm hover:shadow-md transition-all h-full flex flex-col"
                  >
                    {/* 제목 영역과 해결 수 */}
                    ...
                    {/* 태그 영역 - 리스트 뷰와 동일한 순서로 변경: 카테고리 -> 난이도 -> 유형 -> 상태 */}
        ...
        {/* 문제가 없는 경우 - 별도 컴포넌트로 분리하여 레이아웃 일관성 유지 */}
        {problemData?.empty && (
        ...
        <div className="py-6 h-[80px] flex items-center justify-center">
          {problemData && !problemData.empty && (
            <div className="flex justify-center flex-wrap gap-1 sm:gap-2">
              ...
              {/* 페이지 번호 (모바일에서는 현재/전체 형식으로) - 1-based로 표시 */}
              ...
              {/* 페이지 번호 버튼 (데스크톱에서만 표시) - 1-based로 표시 */}
